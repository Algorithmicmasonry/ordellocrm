...
export default async function ProductPricingPage({
    ...
    db.product.findUnique({
      ...
      include: {
        ...
        batches: {
          ...
          include: {
            expenses: {
              where: { type: { in: ["clearing", "waybill"] } },
  ...
  // Build per-currency landed cost from the most recent batch that has a
  // clearing expense in that currency. Waybill from the same batch is
  // included if present; if not, the clearing cost still shows.
  const latestClearingByCurrency = (() => {
    const result: Record<string, { clearing: number; waybill: number }> = {};
    if (!product) return result;

    for (const batch of product.batches) {
      const clearing = batch.expenses.filter((e) => e.type === "clearing");
      const waybill = batch.expenses.filter((e) => e.type === "waybill");

      for (const exp of clearing) {
        if (!result[exp.currency]) {
          const matchedWaybill = waybill.find((w) => w.currency === exp.currency);
          result[exp.currency] = {
            clearing: exp.amount / batch.quantity,
            waybill: matchedWaybill ? matchedWaybill.amount / batch.quantity : 0,
          };
        }
      }
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ProductPricesList
        ...
        clearingByCurrency={latestClearingByCurrency}
      />
