...
import { Plus, Pencil, Trash2, AlertCircle, PackageSearch } from "lucide-react";
...
interface ProductPricesListProps {
  ...
  prices: ProductPrice[];
  clearingByCurrency?: Record<string, { clearing: number; waybill: number }>;
}
          ...
          {prices.length === 0 ? (
            ...
            <div className="border rounded-lg overflow-hidden">
              <Table>
                ...
                <TableBody>
                  {prices.map((price) => {
                    const clearing = clearingByCurrency[price.currency];
                    const landedCost = clearing && (clearing.clearing > 0 || clearing.waybill > 0)
                      ? price.cost + clearing.clearing + clearing.waybill
                      : null;
                        ...
                        <TableCell className="text-right">
                          {landedCost !== null ? (
                            <span className="font-medium text-amber-600 dark:text-amber-400">
                              {getCurrencySymbol(price.currency)}
                              {landedCost.toLocaleString(undefined, {
                                minimumFractionDigits: 0,
                                maximumFractionDigits: 2,
                              })}
                            </span>
                          ) : (
                            <span className="text-muted-foreground text-xs">No clearing</span>
                          )}
                        </TableCell>
                        <TableCell className="text-right">
                          <span
